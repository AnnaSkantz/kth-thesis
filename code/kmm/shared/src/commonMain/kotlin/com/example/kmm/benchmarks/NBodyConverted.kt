...
internal class Body2 {
    ...
    fun offsetMomentum(px: Double, py: Double, pz: Double): Body2 {
        vx = -px / SOLAR_MASS
        vy = -py / SOLAR_MASS
        vz = -pz / SOLAR_MASS
        return this
...
internal class NBodySystem {
    private val bodies: Array<Body2>

    init {
        bodies = arrayOf(Body2.sun(), Body2.jupiter(), Body2.saturn(), Body2.uranus(), Body2.neptune())
...
class NBodyConverted {

    fun runBenchmark(n: Int) {
        val bodies = NBodySystem()
        println("Energy before: " + bodies.energy())
        for (i in 0 until n)
            bodies.advance(0.01)
        println("Energy after: " + bodies.energy())
    }
}