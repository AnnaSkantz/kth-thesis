...
data class Body1(var x: Double = 0.0, var y: Double = 0.0, var z: Double = 0.0,
                 var vx: Double = 0.0, var vy: Double = 0.0, var vz: Double = 0.0,
                 var mass: Double = 0.0)

class BodySystem {
    //Array of celestial bodies: Sun, Jupiter, Saturn, Uranus and Neptune
    private var bodies: Array<Body1> = arrayOf(
        Body1(mass = SOLAR_MASS),
        Body1(4.84143144246472090e+00,
...
class NBodyIdiomatic {

    fun runBenchmark(n: Int) {
        val system = BodySystem()
        println("Energy before: " + system.energy())
        repeat(times = n) {
            system.advance(0.01)
        }
        println("Energy after: " + system.energy())
    }
}